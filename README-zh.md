...
def agent_loop(messages):
    """
    Agent 的核心循环：让 AI 不断思考并执行工具，直到任务完成
    
    参数:
        messages: 对话历史列表，包含用户消息和 AI 回复
                 格式: [{"role": "user"/"assistant", "content": "..."}, ...]
    """
    # 无限循环：让 agent 持续思考和行动，直到任务完成
    while True:
        # 第 1 步：发送消息给 Claude 模型，获取回复
        # client 是 Anthropic API 客户端，messages 是对话历史
        # MODEL 是模型名称（如 claude-3-5-sonnet-20241022）
        # SYSTEM 是系统提示词，定义 agent 的行为规则
        # TOOLS 是可用工具列表（如 bash、read、write 等）
        response = client.messages.create(
            model=MODEL, system=SYSTEM,
            messages=messages, tools=TOOLS,
        )
        
        # 第 2 步：将 AI 的回复添加到对话历史
        # 这样下一次循环时，AI 能"记得"自己说过什么
        messages.append({"role": "assistant",
                         "content": response.content})

        # 第 3 步：检查 AI 是否停止生成
        # stop_reason 告诉我们为什么 AI 停止生成：
        ...
        # 第 4 步：执行 AI 调用的工具
        # 如果 AI 决定使用工具，我们需要在循环中执行它们
        results = []
        
        # 遍历 AI 回复中的所有内容块（block）
        for block in response.content:
            # 找到工具调用的块
            if block.type == "tool_use":
                # 从工具注册表中找到对应的处理函数并执行
                # block.name 是工具名（如 "bash"、"read_file"）
                # block.input 是工具的参数（如 {"command": "ls -la"}）
                # **block.input 将字典展开为关键字参数
                output = TOOL_HANDLERS[block.name](**block.input)
                
                # 收集工具执行结果
                results.append({
                    "type": "tool_result",
                    "tool_use_id": block.id,  # 告诉 AI 这是哪个工具调用的结果
                    "content": output,         # 工具的实际输出
                })
        
        # 第 5 步：将工具执行结果作为"用户消息"加回对话
        # 这样 AI 可以"看到"工具的输出，然后继续思考下一步
        # 注意：这里的 role 是 "user"，模拟用户回复 AI
        messages.append({"role": "user", "content": results})
        
        # 循环继续：回到第 1 步，AI 会根据工具结果继续思考
```
...
python agents/anthropic/s01_agent_loop.py       # 从这里开始
python agents/anthropic/s12_worktree_task_isolation.py  # 完整递进终点
python agents/anthropic/s_full.py               # 总纲: 全部机制合一
```
...
|
|-- agents/                        # Python 参考实现
|   |-- anthropic/                 # Anthropic API 实现 (s01-s12 + s_full)
|   |-- history/                  # 历史版本实现 (LangChain 等)
|   |-- *.py                      # 共享模块 (tools, skills, task_manager 等)
|-- docs/{en,zh}/                 # 心智模型优先的文档 (英/中文)
|-- skills/                       # s05 的 Skill 文件示例
