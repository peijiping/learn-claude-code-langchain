...
- **工具统一走 ToolRegistry**：`agents/tools.py` 的 `TOOL_REGISTRY` 单例统一管理所有工具（原 `tool_base.py` 已合并删除）。工具定义用 `TOOL_REGISTRY.main_agent_tools`（子智能体用 `base_tools`）、处理器用 `TOOL_REGISTRY.handlers`、执行用 `TOOL_REGISTRY.execute(name, **args)`；基础工具方法（`run_bash` / `run_read` / `run_write` / `run_edit` / `run_glob` / `safe_path`）与 todo/background holder（`set_todo_manager` / `get_todo_manager` / `set_background_manager`）均通过该实例调用，禁止再 import 被删除的 `tool_base`。
