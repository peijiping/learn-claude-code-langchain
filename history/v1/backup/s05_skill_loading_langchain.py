#!/usr/bin/env python3
"""
s05_skill_loading_langchain.py - skill技能加载

两层技能注入方法，避免系统提示膨胀：

    第一层（低成本）：系统提示中仅放置技能名称（每个技能约100 tokens）
    第二层（按需加载）：在 tool_result 中加载完整技能内容
    ...
    | tool_result:                         |
    | <skill>                              |
    |   Full PDF processing instructions   |  <-- 第二层：完整内容
    |   Step 1: ...                        |
    |   Step 2: ...                        |
    | </skill>                             |
    +--------------------------------------+

核心洞察："不要把所有内容都塞进系统提示词，按需加载即可。"
"""
...
from tools import CHILD_TOOLS, PARENT_TOOLS, TOOL_HANDLERS, WORKDIR, SKILL_LOADER
...
## 核心能力
- 使用 todo 工具规划多步骤任务（标记 in_progress/completed）
- 使用 task 工具自动分发子任务给子智能体
- 优先使用工具而非纯文本回复

## 何时使用 task 工具（子智能体）
当遇到以下情况时，**应主动调用 task 工具**，无需用户明确要求：
...
**原则**：如果你判断某个子任务**可能需要多次工具调用**或**答案相对独立**，就应使用 task 工具。

**task 工具特点**：子智能体拥有独立上下文，不污染主对话，只返回最终摘要。
"""
# 子智能体系统prompt
