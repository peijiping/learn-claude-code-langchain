#!/usr/bin/env python3
"""
s06_context_compact_langchain.py - 上下文压缩
...
核心洞察："智能体可以有策略地遗忘，从而永远保持工作能力。"

目前实现的的上下文压缩只有第1层，即工具消息过长时，替换历史工具消息为占位符"[之前：使用了 {tool_name}]"；还有窗口滑动。
并没有实现长期消息的提炼和压缩。稍后实现。
