#!/usr/bin/env python3
"""
s09_agent_teams_langchain.py - Agent Teams

持久的命名代理，使用基于文件的 JSONL 收件箱。每个团队成员在单独的线程中运行
自己的代理循环。通过仅追加的收件箱进行通信。
...
from tools import CHILD_TOOLS, PARENT_TOOLS, TOOL_HANDLERS, WORKDIR, SKILL_LOADER, BACKGROUND_MANAGER, BUS, TEAM
